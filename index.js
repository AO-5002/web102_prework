...
function addGamesToPage(games) {
  ...
  games.forEach(function (objectItem) {
    const display = `
    <div class="game-card">
    <h1>${objectItem.name}</h1>
    <p>${objectItem.description}</p>
    <img class="game-img" src=${objectItem.img} />
    </div>
    `;
...
function filterFundedOnly() {
  ...
  const arrFunded = GAMES_JSON.filter((objectItem) => {
    return objectItem.pledged >= objectItem.goal;
  });

  addGamesToPage(arrFunded);
...
const addFirstDisplay = `<h1>${firstGame.name}</h1>`;
const addSecondDisplay = `<h1>${secondGame.name}</h1>`;
firstGameContainer.innerHTML += addFirstDisplay;
secondGameContainer.innerHTML += addSecondDisplay;
